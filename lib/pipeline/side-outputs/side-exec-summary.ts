 ...
 *
 * （2026-09-25）逐条拼接的口径下沉到 `services/voice/speech-lines.ts`：
 * 口播跨段收敛层（`services/voice/condense.ts`）要按**同一口径**定位重复片段，
 * 两处各写一套必然漂移，故单一实现在服务层，此处只做编排。
 */
export function syncNarration(exec: ExecutiveSummary): ExecutiveSummary {
  ...
  out.spoken_must_read = mr.length
    ? mr
        .map((m, i) =>
          mustReadSpeechLine(m, i === 0 ? "" : MR_CONNECTORS[(i - 1) % MR_CONNECTORS.length]),
        )
        .join("")
    : undefined;
  ...
  if (exec.risk) {
    out.spoken_risk = riskSpeechLine(exec.risk);
  } else {
