/**
 * 共享词表层：渲染语言（REPORT_LOCALE）原语。
 *
 * 2026-09-14 Phase 2b 自 `render/locale.ts` 下沉至此，解开 assemble → render 反向依赖。
 * gzinfo 从 env 直读；2.0 服务层禁止读 env —— 由组合根/scripts 启动时调
 * `setReportLocale(process.env.REPORT_LOCALE)` 写入。ESM live binding：各模块
 * `import { REPORT_LOCALE }` 会看到更新后的值。
