...
export const THEME_INTERACTIVE_CSS = `  /* ===== 2026-08-21 交互重构（demo 对齐）：报头 / 单层 tab / 卡片徽章 / 商机默认展开 / 字号体系 ===== */
  ...
  /* 合成后端徽标（2026-08-24；2026-09-21 增补百炼）：腾讯云=蓝（云），百炼=橙（阿里云），开源 Piper=灰 */
  .player-badge {
  ...
  .player-badge-cosyvoice {
    color: #c2410c;
    background: color-mix(in srgb, #c2410c 12%, var(--bg));
    border-color: color-mix(in srgb, #c2410c 38%, var(--rule));
  }
  .player-badge-piper {
    color: var(--muted);
