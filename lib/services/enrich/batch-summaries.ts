...
import { REPORT_LOCALE } from "../render/locale";

/**
 * 批量摘要富集（gzinfo lib/ai/enrich.ts 逐字移植，2026-09-13）。
 *
 * 2.0 适配：
 *  - `runLlm` 直连 → 调用方注入 `runner`（组合根装配，服务层零 SDK）；
 *  - REPORT_LOCALE → `../render/locale`（live binding，入口脚本注入）；
 *    prompt 选择从「模块加载期定格」改为**调用期读取**（注入先于调用即可生效）；
 *  - 「少回」诊断落盘 → `adapters/llm-log.ts` 的 dumpEnrichUndercount（服务层零 fs）。
 */
export type BatchEnrichRunner = (systemPrompt: string, userPrompt: string) => Promise<string>;

/** 「少回」诊断回调（由调用方注入 IO 实现，如 adapters/llm-log 的 dumpEnrichUndercount）。 */
export interface EnrichDiag {
  dumpRaw?(info: { scope: string; rawText: string; requested: number; returned: number }): void;
}
...
// 按当前 REPORT_LOCALE（live binding）在调用期选择 prompt 集；各 enricher 经 prompts().<key> 取用。
const prompts = () =>
  REPORT_LOCALE === "en"
...
async function runEnrichment(
  ...
  try {
    const text = await runner(systemPrompt, userPrompt);
    ...
    if (result.size < payload.length / 2 && payload.length >= 3) {
      diag?.dumpRaw?.({ scope, rawText: text, requested: payload.length, returned: result.size });
    }
  } catch (e) {
...
export async function enrichGithubTrendingSummaries(
  ...
  return runEnrichment(payload, prompts().gh, "GH summaries", runner, opts);
}
...
export async function enrichFinanceNewsSummaries(
  ...
  return runEnrichment(payload, prompts().finance, "finance summaries", runner, opts);
}
...
export async function enrichXViralSummaries(
  ...
  return runEnrichment(payload, prompts().xViral, "X-viral summaries", runner, opts);
}
...
export async function enrichTrendingPapersSummaries(
  ...
  return runEnrichment(payload, prompts().papers, "papers summaries", runner, opts);
}
