 ...
 * 制度/领域泛称候选（不算机构）。
 *
 * 2026-09-16 修复：INST_RE 会把「存款保险 / 农业保险」这类**制度领域词**提取成机构
 * （实测《存款保险条例实施情况评估报告》《存款保险宣传周》《农业保险承保理赔管理办法》
 * 全被误杀——存款保险是央行核心制度、银行日报核心题材）。
 *
 * ⚠️ 为什么不用「掩码」：掩码会连公司名里的字一起吞掉（「安华农业保险获批增资」→
 * 机构丢失）。故在**候选级**判定：仅当候选是裸词、或带通用前缀（我国/全国/政策性）时
 * 才豁免；带真实公司前缀（安华/阳光/中原农业保险…）仍按机构处理。
 */
const DOMAIN_TERM_RE = /^(?:我国|全国|政策性)?(?:存款保险|农业保险)$/;
...
function extractInstitutions(text: string): string[] {
  ...
  while ((m = INST_RE.exec(masked))) {
    ...
    // 制度/领域泛称（存款保险/农业保险…）：裸词或带通用前缀 → 不算机构
    if (DOMAIN_TERM_RE.test(name)) continue;
