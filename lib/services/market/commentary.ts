...
/**
 * 2.0 适配：
 *  - `runLlm` 直连 → 调用方注入 `LlmRunner`（组合根装配，服务层零 SDK 依赖）；
 *  - `locale` env → 由调用方注入 `locale`；
 *  - **剥离 crypto**：gzinfo 的 `cryptoFearGreed` / `cryptoGlobal` 输入与相关
 *    prompt 段落全部移除（2026-09-12 硬性规定：加密资产永久剔除）。
 */
...
4. market_overview must cover all 3 asset categories (US equity / China-HK equity / commodities-FX).
...
const systemPromptOf = (locale: "zh" | "en"): string =>
  locale === "en" ? SYSTEM_PROMPT_EN : SYSTEM_PROMPT_ZH;

export async function generateTradingCommentary(
  input: TradingCommentaryInput,
  runner: TradingLlmRunner,
  locale: "zh" | "en" = DEFAULT_LOCALE,
  ...
  // 加密上下文：gzinfo 此处会拼接「加密恐慌贪婪指数 / 加密总市值」两行，
  // 2.0 按硬性规定（加密资产永久剔除）**整体移除**，故 contextLines 恒为空。
  ...
  const userPrompt =
    locale === "en"
      ? [
          ...
          .join("\n")
      : [
          `**输出硬约束**：响应必须是单一合法 JSON 对象（以 \`{\` 开头以 \`}\` 结尾，不要 markdown、不要前后缀）。**watchlist 字段必须正好包含 3-5 个完整的 WatchlistPick 对象**，每个对象形如 \`{ "symbol": "...", "display_name": "...", "stance": "偏上行"|"偏下行"|"中性", "rationale": "80-150 字中文，引用具体指标数字" }\`。**禁止把 watchlist 写成 ticker symbol 字符串数组**（如 \`["^TNX","SPY"]\` 是错的）——这是技术指标解读任务，每条必须含 rationale 字段。空数组或字符串数组都是输出错误。`,
          "",
          contextLines.length > 0
